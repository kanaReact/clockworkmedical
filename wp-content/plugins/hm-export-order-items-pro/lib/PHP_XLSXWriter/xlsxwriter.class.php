<?php
/*
See ./LICENSE for the copyright and license applicable to the original version of this file.
This file has been modified from the original.
In this project, the GNU General Public License version 3 or later applies.
*/

class AGS_XOIWCP_XLSXWriter
{
	...
	protected function initializeSheet($sheet_name, $col_widths=array(), $auto_filter=false, $freeze_rows=false, $freeze_columns=false )
	{
		...
		$this->sheets[$sheet_name] = (object)array(
			...
			'row_count' => 0,
			'file_writer' => new AGS_XOIWCP_XLSXWriter_BuffererWriter($sheet_filename),
			'columns' => array(),
			'merge_cells' => array(),
	...
	protected function writeCell(AGS_XOIWCP_XLSXWriter_BuffererWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx)
	{
	...
	protected function writeStylesXML()
	{
		...
		$file = new AGS_XOIWCP_XLSXWriter_BuffererWriter($temporary_filename);
...
class AGS_XOIWCP_XLSXWriter_BuffererWriter
{
	protected $fd=null;
	protected $buffer='';
	...
	public function __construct($filename, $fd_fopen_flags='w', $check_utf8=false)
	{
		...
		if ($this->fd===false) {
			AGS_XOIWCP_XLSXWriter::log("Unable to open $filename for writing.");
		}
	}
	...
	protected function purge()
	{
		if ($this->fd) {
			if ($this->check_utf8 && !self::isValidUTF8($this->buffer)) {
				AGS_XOIWCP_XLSXWriter::log("Error, invalid UTF8 encoding detected.");
