...
class GP_Perk {
	...
	public static function get_perk( $perk_file ) {
		...
		if ( ! class_exists( $perk_class ) ) {
			...
			if ( ! file_exists( $perk_path ) ) {
				return new WP_Error( 'perk_file_error', __( 'The file for this perk does not exist.', 'spellbook' ) );
			}
			...
			if ( ! class_exists( $perk_class ) ) {
				...
				if ( ! class_exists( $perk_class ) ) {
					return new WP_Error( 'perk_class_error', __( 'There is no class for this perk.', 'spellbook' ) );
				}
			}
		}
		...
		if ( ! is_a( $perk, 'GP_Perk' ) ) {
			return new WP_Error( 'not_a_perk', sprintf( __( '%s is not a perk.', 'spellbook' ), $perk_file ) );
		}
	...
	public function requirements_admin_notice() {
		...
		<div class="error notice gp-requirements-notice">
			<p>
				...
				<strong><?php printf( __( 'Uh-oh! <strong>%s</strong> needs your attention.', 'spellbook' ), $this->data['Name'] ); ?></strong>
			</p>
	...
	*/
	...
	public function delete() {
		...
		if ( ! $perk_dir ) {
			return new WP_Error( 'perk_delete', __( 'There was an issue deleting this perk. The perk directory was not available.', 'spellbook' ) );
		}
