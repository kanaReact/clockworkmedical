    ...
    protected function safe_render()
    {
        ...
        if ($p_query->have_posts()) {
            ...
            while ($p_query->have_posts()) {
                ...
                echo $id_page_attr;
                ?> column-<?php 
                echo $columns_grid_attr . $classItem;
                ?>  <?php 
                ...
                if ($settings['templatemode_linkable']) {
                    echo '<a style="cursor: pointer; position: absolute; left: 0; top: 0; right: 0; bottom: 0; z-index: 10;" href="' . esc_url(get_the_permalink()) . '">';
                    echo '</a>';
                }
                ?>
						</div><!-- end wrapper -->
            ...
            if ($term_list && \is_array($term_list)) {
                ...
                foreach ($term_list as $term) {
                    ...
                    if (!$termparent || !$settings['only_parent_metadata']) {
                        echo '<' . $tag_metadata . '>';
                        $term_url = esc_url(trailingslashit(get_term_link($term)));
                        $linkOpen = '';
                        $linkClose = '';
        ...
        if (!empty($ACFitems)) {
            foreach ($ACFitems as $acfkey => $acfitem) {
                ...
                if ($acf_i == 'title') {
                    ...
                    $date_formatted = $date_value ? esc_html($date_value) : '';
