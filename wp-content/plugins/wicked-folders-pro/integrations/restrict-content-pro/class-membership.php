...
class Membership extends Stub_Post_Type {

    public $name            = 'Restrict Content Pro Membership';

    public $name_plural     = 'Restrict Content Pro Memberships';

    public $post_type_name  = 'wf_rcp_membership';

    public function __construct() {
        parent::__construct();
        
        add_filter( 'rcp_memberships_list_table_columns',               array( $this, 'add_move_to_folder_column' ) );
        add_filter( 'rcp_memberships_list_table_column_wicked_move',    array( $this, 'move_to_folder_column_content' ), 10, 2 );
        add_filter( 'rcp_memberships_query_clauses',                    array( $this, 'memberships_query_clauses' ) );
    }

    public function is_post_type_screen() {
        return isset( $_GET['page'] ) && 'rcp-members' == $_GET['page'] && ! isset( $_GET['view'] );        
    }
    ...
    public function memberships_query_clauses( $clauses ) {
        global $wpdb;
        ...
        if ( $state->folder ) {
            if ( 'unassigned_dynamic_folder' == $state->folder ) {
                ...
                if ( ! empty( $ids ) ) {
                    $clauses['where'] .= " AND rcp_m.id NOT IN (" . implode( ',', $ids ) . ") ";
                }
            } else {
                $clauses['join']    .= " INNER JOIN {$wpdb->term_relationships} AS wf_tr ON wf_tr.object_id = rcp_m.id ";
    ...
    public function get_assigned_ids() {
        ...
        $ids = $wpdb->get_col(
            $wpdb->prepare(
                "
                    SELECT DISTINCT rcp_m.id FROM {$wpdb->prefix}rcp_memberships rcp_m
                    INNER JOIN {$wpdb->term_relationships} AS wf_term_relationships ON wf_term_relationships.object_id = rcp_m.id
    ...
    public function get_total_count() {
        ...
        return $wpdb->get_var( "SELECT COUNT(id) FROM {$wpdb->prefix}rcp_memberships" );
    }
}