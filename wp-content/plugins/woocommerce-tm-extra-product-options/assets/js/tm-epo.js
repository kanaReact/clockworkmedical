( function( window, document, $ ) {
	...
	function repeaterAdd( el, elementInnerWrap ) {
		...
		clone.find( '.tmcp-date-select' ).attr( 'id', function( i, value ) {
			return $( this ).attr( 'data-tm-date' ) + '_' + index + '_' + value.substring( value.lastIndexOf( '_' ) + 1 );
		} );
	...
	function repeaterDeleteSection( elementInnerWrap, repeaterElement, repeaterObj, $this ) {
		...
		elementInnerWrap.find( '.tc-repeater-section' )
			...
			.forEach( function( clone, index ) {
				...
				clone.find( '.tmcp-date-select' ).attr( 'id', function( i, value ) {
					return $( this ).attr( 'data-tm-date' ) + ( ( index !== 0 ) ? '_' + index : '' ) + '_' + value.substring( value.lastIndexOf( '_' ) + 1 );
				} );
				clone.find( '[data-tm-date]' ).attr( 'data-tm-date', function( i, value ) {
					return value.replace( /\_\d+$/g, '' ) + ( ( index !== 0 ) ? '_' + index : '' );
				} );
					...
					if ( qtyElement.length ) {
						qtyElement
							...
							.on( 'change.rs' + qx + 'cpf input.rs' + qx + 'cpf', function( event ) {
								var field = $( this );
								var prevValue = field.data( 'tm-prev-valuers' + qx );
								var value = $.epoAPI.applyFilter( 'qtyElementForRepeaterQuantityValue', field.val(), {
									epo: epoObject,
									currentCart: currentCart,
									obj: obj,
								...
								if ( qx === quantityRepeaters.length ) {
									field.data( 'prev-valuers' + qx, fieldprevval );
								}
								if ( difference > 0 ) {
						...
						qtyElement.data( 'tm-prev-value' + qx, $.epoAPI.applyFilter( 'qtyElementForRepeaterQuantityValue', qtyElement.val(), {
							epo: epoObject,
							currentCart: currentCart,
							obj: obj,
							qtyElement: qtyElement,
