 ...
 */
class GF_Stripe_Billing_Portal {
	...
	*/
	...
	private function get_email_subscriptions( $email ) {
		...
		foreach ( $entry_ids as $entry_id ) {
			...
			$subscription['frequency']  = $this->build_subscription_frequency_string( $stripe_subscription );
	...
	/**
	 * Builds the frequency string from plan interval and interval count properties.
	 *
	 * @since 5.9.2
	 *
	 * @param $subscription \Stripe\Subscription The subscription object.
	 *
	 * @return string The frequency string.
	 */
	protected function build_subscription_frequency_string( $subscription ) {
		$interval       = $subscription->plan->interval;
		$interval_count = $subscription->plan->interval_count;

		$labels = array(
			/* Translators: days count */
			'day'   => array( 'singular' => __( 'daily', 'gravityformsstripe' ), 'plural' => __( 'every %s days', 'gravityformsstripe' ) ),
			/* Translators: weeks count */
			'week'  => array( 'singular' => __( 'weekly', 'gravityformsstripe' ), 'plural' => __( 'every %s weeks', 'gravityformsstripe' ) ),
			/* Translators: months count */
			'month' => array( 'singular' => __( 'monthly', 'gravityformsstripe' ), 'plural' => __( 'every %s months', 'gravityformsstripe' ) ),
			/* Translators: years count */
			'year'  => array( 'singular' => __( 'yearly', 'gravityformsstripe' ), 'plural' => __( 'every %s years', 'gravityformsstripe' ) ),
		);

		if ( 1 === (int) $interval_count ) {
			return $labels[ $interval ]['singular'];
		}

		return sprintf( $labels[ $interval ]['plural'], $interval_count );
